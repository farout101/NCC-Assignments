...
void appendFile(const char *file_name, const char *username, const char *hashed_password, int user_point)
{
   ...
   fprintf(file, "%d. %s %s %d\n", userCount, username, hashed_password, user_point);
...
void loadUserData(const char *file_name)
{
   ...
   while (fscanf(file, "%s", action) != EOF)
   {
      if (strcmp(action, "transfer") == 0)
      {
         ...
         if (sourceUser != NULL && destinationUser != NULL)
         {
            // Handle the transfer logic (update user points, etc.)
            printf("Transfer: %s transferred %d points to %s\n", sourceUser->name, amount, destinationUser->name);
         }
         else
         {
   ...
   if (userCount < 50)
   {
      ...
      else
      {
         ...
         if (strcmp(password, confirmPass) == 0)
         {
            ...
            appendFile("Data.txt", users[userCount - 1].name, users[userCount - 1].password, users[userCount - 1].credits);
         }
         else
         {
...
void createTestUsers()
{
   char testUsernames[5][50] = {"user1@test.com", "user2@test.com", "user3@test.com", "user4@test.com", "user5@test.com"};
   char testPasswords[5][50] = {"Password1", "Password2", "Password3", "Password4", "Password5"};

   for (int i = 0; i < 5; i++)
   {
      if (userCount < 50)
      {
         strcpy(users[userCount].name, testUsernames[i]);
         strcpy(users[userCount].password, testPasswords[i]);
         users[userCount].credits = INITIAL_CREDITS;
         userCount++;
      }
      else
   ...
   printf("Test users created successfully!\n");
}
